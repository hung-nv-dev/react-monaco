...
import type { SOCQueryEditorProps, SOCQueryEditorRef, SavedQuery, DataSource } from './SOCQueryEditor.types';
import './SOCQueryEditor.css';

let initialized = false;

const SOCQueryEditorComponent = forwardRef<SOCQueryEditorRef, SOCQueryEditorProps>(
  (
    {
    ...
    useEffect(() => {
      ...
      const calculateSuggestionPosition = (): boolean => {
        ...
        if (suggestWidget) {
          ...
          // If suggestion would go below viewport, position it above the cursor
    ...
    useImperativeHandle(ref, () => ({
      ...
      insertText: (text: string) => {
        if (editorRef.current) {
          ...
          if (selection) {
            editorRef.current.executeEdits('insert', [{ range: selection, text, forceMoveMarkers: true }]);
          }
      ...
      validate: () => {
        ...
        setErrors(result.errors);
        if (editorRef.current) setValidationMarkers(editorRef.current.getModel()!, result.errors);
    ...
    return (
      <div className={`soc-query-editor ${className} ${disabled ? 'soc-query-editor--disabled' : ''}`} style={style}>
        {showToolbar && (
          <EditorToolbar
        ...
        <div className={`soc-query-editor__container ${isFocused ? 'soc-query-editor__container--expanded' : ''}`}>
          {(loading || isSearching) && (
            <div className="soc-query-editor__loading">
              <Spin tip={isSearching ? 'Searching...' : undefined} />
            </div>
          )}
          <div
            ref={containerRef}
            className={`soc-query-editor__monaco ${isFocused ? 'soc-query-editor__monaco--expanded' : ''}`}
            style={{ height: `${editorHeight}px`, transition: 'height 0.2s ease' }}
          />
        </div>
...
SOCQueryEditorComponent.displayName = 'SOCQueryEditor';

export const SOCQueryEditor = SOCQueryEditorComponent;
